...
export const orderItemSchema = z.object({
  ...
  variantDetails: z.string().max(200).optional(),
...
export const createOrderSchema = z.object({
  ...
  customerPhone: z.string().min(10, "Valid phone number required").max(15, "Phone number is too long").trim(),
  customerEmail: z.string().email("Invalid email format").optional().or(z.literal("")),
  shippingAddress: z.string().min(5, "Delivery address must be at least 5 characters").max(300, "Address is too long").trim(),
